        ...
        {view === 'match' && (
          <>
            ...
            {docHeader && (docHeader.title || (docHeader.lines && docHeader.lines.length > 0)) && (
              ...
              >
                {docHeader.title && (
                  <Typography variant="h6" sx={{ fontWeight: 900, color: '#01579b', mb: 1, textTransform: 'uppercase', letterSpacing: '0.02em' }}>
                    {docHeader.title}
                  </Typography>
                )}
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                  {docHeader.lines
                    ?.filter((line, index, arr) => {
                      ...
                      // Remove CST No and DL No if they are at the end
                      const lower = line.toLowerCase();
                      if (index >= arr.length - 2 && (lower.includes('cst no') || lower.includes('d.l. no'))) return false;
                      return true;
                    })
                    .map((line, idx) => {
